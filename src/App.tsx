...
function App() {
    ...
    useEffect(() => {
        try {
            fetch(
                `https://gnews.io/api/v4/top-headlines?token=${NEWSAPIKEY}&topic=technology&&country=us&language=en`
            )
                .then((res) => {
                    if (res.status !== 200) {
                        throw Error(
                            "Fetching headlines data available on localhost only."
                        );
                    }
                    return res.json();
                })
    ...
    useEffect(() => {
        ...
        Promise.all([
            fetch(
                `${BASEURL}/${arr[0]}/${yearStr}-${monthStr}-${dayStr}?apiKey=${POLYGONAPIKEY}`
            ).then((res) => res.json()),
            fetch(
                `${BASEURL}/${arr[1]}/${yearStr}-${monthStr}-${dayStr}?apiKey=${POLYGONAPIKEY}`
            ).then((res) => res.json()),
            fetch(
                `${BASEURL}/${arr[2]}/${yearStr}-${monthStr}-${dayStr}?apiKey=${POLYGONAPIKEY}`
            ).then((res) => res.json()),
            fetch(
                `${BASEURL}/${arr[3]}/${yearStr}-${monthStr}-${dayStr}?apiKey=${POLYGONAPIKEY}`
            ).then((res) => res.json()),
            fetch(
                `${BASEURL}/${arr[4]}/${yearStr}-${monthStr}-${dayStr}?apiKey=${POLYGONAPIKEY}`
            ).then((res) => res.json()),
        ]).then((res) => {
            if (res[0].status === "ERROR") {
                setMessage(
                    "API request limit has been reached. Try refreshing page in 30-60 seconds."
                );
            }
        ...
        <div>
            ...
            <main>
                ...
                <p className="subheader">
                    Rearrange the list of company's stock price from highest to
                    lowest.
                </p>
                    ...
                    {reveal && (
                        <div className="answers">
                            ...
                            <small className="last-update">
                                ...
                                from <a href="https://polygon.io">Polygon</a>{" "}
                            </small>
                        </div>
                    )}
                </div>
